...
// EDIT domain ke proxy Vercel: e.g. "https://website-omega-ten-69.vercel.app/api/proxy"

(function(global){
  const WANZ_CONFIG = {
    // full base URL (no trailing slash), gunakan proxy Vercel jika backend HTTP
  ...
  async function safeFetch(path="/", opts = {}){
    if (!BASE) return { success:false, message:"WANZ_CONFIG.domain belum diset", code:"NO_DOMAIN" };
    let p = path || "/"; if (!p.startsWith("/")) p = "/" + p;
  ...
  const Wanzdev = {
    ...
    async connect(name, phone){ if (!name||!phone) return { success:false, message:"name & phone required" }; return await safeFetch("/connect", { method:"POST", body:{ name, phone } }); },
    async disconnect(name){ if (!name) return { success:false, message:"name required" }; return await safeFetch("/disconnect", { method:"POST", body:{ name } }); },
    async sendMessage(name, to, text){ if (!name||!to||!text) return { success:false, message:"name,to,text required" }; return await safeFetch("/send", { method:"POST", body:{ name, to, text } }); },
